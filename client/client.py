...
# import getch
# import getch # pip3 install getch
...
def run(username, stub):
    while (True):
        ...
        if menu=="1":
            while True:
                to_user = input("To  : ")
                m = helloworld_pb2.Request(username="empty")
                ...
                curr_mensaje = helloworld_pb2.Text(from_user=username, to_user = to_user,text = body, date=date)
                response = stub.SendText(curr_mensaje)
                # print("\t(Press any key to continue)"); getch.getch()
                break;

        if menu == "2":
            m = helloworld_pb2.Request(username=username)
